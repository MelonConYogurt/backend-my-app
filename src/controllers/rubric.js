...
RubricRouter.post("/create", async (req, res) => {
  try {
    const { title, description, docentId } = req.body;
    ...
    const newRubric = new Rubric({ title, description, docentId });
...
RubricRouter.patch("/update/:id", async (req, res) => {
  try {
    ...
    const updatedRubric = await User.findByIdAndUpdate(rubricId, req.body, {
      new: true,
      runValidators: true,
    });
    ...
    if (!updatedRubric) {
      return res.status(404).json({
        message: "No se encontrola rubrica",
      });
    }

    return res.status(200).json({
      message: "Rubrica actualizada correctamente",
      data: updatedRubric,
    });
  } catch (error) {
    return res.status(500).json({
      message: "Error al actualizar la rubrica",
      error: error.message,
    });
  }
});
